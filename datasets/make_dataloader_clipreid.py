...
def make_dataloader(cfg):
    ...
    #定义了训练集数据的预处理和增强操作，包括调整尺寸、水平翻转、填充、随机裁剪、归一化以及随机擦除等，旨在增强模型的泛化能力。
...
#定义了验证集数据的预处理，主要包括调整尺寸、转为张量格式和归一化，适用于验证阶段，不包括数据增强操作
    ...
    view_num = dataset.num_train_vids#视角个数
    ...
    train_loader_stage1 = DataLoader(
        train_set_normal, batch_size=cfg.SOLVER.STAGE1.IMS_PER_BATCH,
        sampler=RandomIdentitySampler(dataset.train, cfg.SOLVER.STAGE1.IMS_PER_BATCH, cfg.DATALOADER.NUM_INSTANCE),
        num_workers=num_workers, collate_fn=train_collate_fn
    )
